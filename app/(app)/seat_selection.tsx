    ...
    const showAlert = (
        ...
        const handleCheckout = async () => {
            if (selectedSeats.length === 0) {
                showAlert("Pilih Kursi", "Minimal pilih 1 kursi dulu dong!");
                return;
            }
            ...
            try {
                ...
                console.log("Mengirim Data:", orderData);

                const response = await fetch(
                    'https://ubaya.cloud/react/160422148/uas/create_order.php',
                    {
                        method: 'POST',
                ...
                if (json.result === 'success') {
                    showAlert(
                        "Berhasil!",
                        "Tiket berhasil dibeli. Cek Riwayat ya!",
                        () => router.replace("/(app)/order_history")
                    );
                } else {
                    showAlert("Gagal", json.message || "Transaksi gagal.");
                }

            } catch (error) {
                console.error(error);
                showAlert("Error Fatal", "Gagal menghubungi server.");
            }
        };
